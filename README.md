...
- Postgres: `pgdata` (named volume)
- Redis: `redisdata` (named volume)
- Static files: `./staticfiles` (bind mount - allows native nginx to serve files)
- Media files: `./media` (bind mount)

**Important:** Static files use a bind mount instead of a named volume. This allows:
- Native nginx on the host to serve static files directly (production setups with Certbot)
- Easy inspection and debugging of collected static files
- Simpler integration with external web servers

The `staticfiles/` directory is auto-generated by `collectstatic` and is gitignored.
